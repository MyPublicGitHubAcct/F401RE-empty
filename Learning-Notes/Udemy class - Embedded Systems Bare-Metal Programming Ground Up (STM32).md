...
The __DMA__ allows data transfers to take place in the background, without intervention of the Cortex-M processor. During this operation, the main processor can execute other tasks and it is only interrupted when a whole data block is available for processing. Large amounts of data can be transferred with no major impact on the system performance.

Some or all of the following will need to be configured:
...
The __DMA__ has two ports; one peripheral port and one memory port.

There are two __DMA__ modules. Each implements 8 different streams. Each stream is dedicated to managing memory access requests from one or more peripherals. Each stream has up to 8 selectable channels (requests) in total, but _only one_ channel, across all streams, can be active at one time.

The streams are pre-configured - see the reference manual.

The __DMA__ has an arbiter for handling the priority between the __DMA__ streams. Stream priority is software-configurable - there are 4 levels. If two or more __DMA__ streams have the same software priority level, the hardware priority is used. The hardware priority is such that stream 0 is highest priority and stream 7 is lowest.

A __DMA__ transfer is defined by a source address and a destination address. Both the source and destination should be in the AHB or APB memory ranges.

__DMA__ is capable of performing three different _transfer modes_:

- Peripheral to memory
- Memory to peripheral
- Memory to memory (only available with DMA2)

The _transfer size_ has to be defined (only) when the __DMA__ is the flow controller. This value defines the volume of data to be transferred from source to destination.

Also, the __DMA__ can _automatically_ increment the source and destination addresses after each transfer, but this must be configured.

_Data width_ options are:

- Byte (8 bits)
- Half-word (16 bits)
- Word (32 bits)

One of two _transfer types_ needs to be configured:

- __Circular mode__ - For handling circular buffers and continuous data flows. The DMA_SxNDTR register is then reloaded automatically with the previously programmed value.
- __Normal mode__ - Once the DMA_SxNDTR register reaches zero, the stream is disabled.

With _FIFO mode_, each stream has an independent 4-word (4 * 32 bits) FIFO. The FIFO is used to temporarily store data coming from the source before transmitting it to the destination. If DMA FIFO is enabled, data packing/unpacking and/or Burst mode can be used. The configured DMA FIFO threshold defines the DMA memory port request time.

_FIFO mode_ reduces SRAM access and so gives more time for the other masters to access the bus matrix without additional concurrency. It also allows software to do burst transactions wihc optimize the transfer bandwidth. Finally, _FIFO mode_ allows packing/unpacking data to adapt source and destination data with with _no extra DMA access_.

### Example - Developing the UART Transmitter DMA driver

Need to determine which DMA module & stream UART is connected to by reading the manual.

You need to disable the stream before changing the memory address and the length of the transfer, since those won't be able to write to them when the stream is enabled.

For more advanced use, see 

- [Working with ADC and STM32 part1: Single Channel Single Conversion](https://blog.embeddedexpert.io/?p=200)
- [Working with ADC and STM32 part2: Single Channel Continuous Conversion](https://blog.embeddedexpert.io/?p=224)
- [Working with ADC and STM32 part3: Single Channel Continuous Conversion with Interrupt](https://blog.embeddedexpert.io/?p=235)
- [Working with ADC and STM32 part 4: Multi Channel Continuous Conversion with DMA](https://blog.embeddedexpert.io/?p=319)
- [Working with ADC and STM32 part 5: Timer triggered with DMA](https://blog.embeddedexpert.io/?p=858)
- [Working with ADC and STM32 part 6: Timer triggered with DMA with Double Buffer](https://blog.embeddedexpert.io/?p=1849)

To test this, press the reset button on the Nucelo board. The LED should already have been lit; on pressing the button, it will stop being lit & a message will be sent via UART (and viewable in CoolTerm).
...
a.k.a, Two Wire Interface (TWI), I2C has two lines:

- _Serial Clock_ (SCL), which is used for synchronizing data transder between master and slave.
- _Serial Data_ (SDA), which is used for transmitting data.

### Operation Modes

- _Master Transmitter_
- _Master Receiver_
- _Slave Transmitter_
- _Slave Receiver_
...
- Transactions are initiated and completed byt he master.
...
![diagram4](I2C-START-STOP-condition.png)

- All transcations begin with START and are terminated by STOP.
...
- Each byte must be followed by and Acknowledge (ACK) bit.
- Data is transferred with the Most Significant Bit (MSB) first.
- The address frame is first in any new communication sequence. Otherwise, the message will not land anywhere.
- For a 7-bit address, the address is sent out MSB first, followed by a R/W bit indicating whether this is a read (1) or write (0) operation.
- The data frame begins transmission after the address frame is sent.
- The master will simply continue generating clock pulses pm SCL at a regular interval, and the data will be placed on SDA by either the master or the slave (depending on whether the R/W bit indicated a read or write operation.

The speed of the I2C interface must correspond with the bus speeds defined in the I2C specification:

- _Standard-mode_ : 100 KHz maximum
- _Fast-mode_ : 400 KHz maximum
- _Fast-mode plus_ : 1 MHz
- _High-speed mode_ : 3.4 MHz

### I2C Duty Cycle

The _duty cycle_ specifies the ratio between t\_LOW and t\_HIGH of the I2C SCL line.

Possible values are:

- *ISC\_DUTYCYCLE\_2* = 2:1
- *ISC\_DUTYCYCLE\_16\_9* = 16:9
...
### Example - Implementing the SPI GPIO Initialization function
...
### Example - Configuring the SPI Parameters
...
### Example - Implementing the SPI Transmit
...
### Example - Implementing the SPI Receive
...
### Example - Configuring the ADLX345 Accelerometer using the SPI driver
...
Alternatively, and particularly because there may be a need to move to a different chip manufacturer / family, this [playlist](https://www.youtube.com/watch?v=uQQsDWLRDuI&list=PLP29wDx6QmW7HaCrRydOnxcy8QmW0SNdQ) is a good example of using [libopemcm3](https://github.com/libopencm3/libopencm3). The instructor, while probably being less knowlegable overall, is a much better teacher & covers things that could/should have been covered above. The source code for the project in the videos can be found [here](https://github.com/lowbyteproductions/bare-metal-series).
